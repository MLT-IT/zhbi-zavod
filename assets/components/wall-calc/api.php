...
if (empty($_SERVER['HTTP_X_REQUESTED_WITH']) || strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) != 'fetch' || empty($query)) {
  die(json_encode(['error' => 'Headers not set or empty query']));
}

$query = preg_replace('/[^a-zA-Z0-9-_а-яА-ЯёЁЇїіІЄєҐґЁё]/u', '',$query);
...
$c->leftJoin('msProductOption', 'item_weight', "item_weight.product_id = msProductData.id AND item_weight.key = 'weight'");
