YandexDiskUploader.grid.Default = function (config) {
    config = config || {};

    if (typeof(config['multi_select']) != 'undefined' && config['multi_select'] == true) {
        config['sm'] = new Ext.grid.CheckboxSelectionModel();
    }
    config['cls'] = (config['cls'] || 'main-wrapper') + ' yandexdiskuploader-grid';
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('yandexdiskuploader-grid__row_disabled');
                }
                return cls.join(' ');
            },
        },
    ...
    YandexDiskUploader.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(YandexDiskUploader.grid.Default, MODx.grid.Grid, {
    ...
    getSearchField: function (config, width) {
        return {
            xtype: 'yandexdiskuploader-field-search',
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = YandexDiskUploader.utils.getMenu(row.data['actions'], this, ids);
    ...
    _doAction: function (action, action_prefix, confirm, check_id) {
        ...
        if (confirm) {
            MODx.msg.confirm(Ext.apply({}, params, {
                title: ids.length > 1
                    ? _('yandexdiskuploader_button_' + action + '_multiple')
                    : _('yandexdiskuploader_button_' + action),
                text: _('yandexdiskuploader_confirm_' + action),
            }));
        } else {
...
Ext.reg('yandexdiskuploader-grid-default', YandexDiskUploader.grid.Default);