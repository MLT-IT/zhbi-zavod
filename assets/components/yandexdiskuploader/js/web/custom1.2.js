...
if (uploaders.length > 0) {
    for (var i = 0; i < uploaders.length; i++) {
        ...
        Dropzones[i] = new Dropzone('.dropzone_' + (i+1), {
            ...
            acceptedFiles: "image/jpeg,image/jpg,image/png,application/pdf,.dwg,.xlsx,.doc,.docx,.xls"
        });
        Dropzones[i].on('success', function(e, response) {
            response = JSON.parse(response);
            let link = "Сайт: krovlya.ru <br> Ссылка: " + response.url;
            document.getElementById(response.inputId).setAttribute('value', link);
        });
        Dropzones[i].on("addedfile", file => {
            document.getElementsByClassName("yaDiskUploader")[0].style.display = "block";
