...
class YandexDiskUploader
{
    ...
    /** @var modX $modx */
    public $modx;
    /** @var yandexdiskuploaderTools $tools */
    public $tools;
    ...
    function __construct(modX &$modx, array $config = array())
    {
        ...
        $corePath = $this->modx->getOption('yandexdiskuploader_core_path', $config, MODX_CORE_PATH . 'components/yandexdiskuploader/');
        $assetsUrl = $this->modx->getOption('yandexdiskuploader_assets_url', $config, MODX_ASSETS_URL . 'components/yandexdiskuploader/');
        $assetsPath = $this->modx->getOption('yandexdiskuploader_assets_path', $config, MODX_ASSETS_PATH . 'components/yandexdiskuploader/');
        ...
        $this->modx->addPackage('yandexdiskuploader', $this->config['modelPath']);
        $this->modx->lexicon->load('yandexdiskuploader:default');
    }
    ...
    public function loadFrontendScripts($objectName = '', array $sp = array())
    {
        ...
        if (empty($this->modx->loadedjscripts[$objectName]) && (!defined('MODX_API_MODE') || !MODX_API_MODE)) {
            ...
            if ($css = trim($this->modx->getOption('yandexdiskuploader_frontend_css'))) {
                $this->modx->regClientCSS(str_replace($pls['pl'], $pls['vl'], $css));
            }
            if ($js = trim($this->modx->getOption('yandexdiskuploader_frontend_js'))) {
    ...
    /**
     * @return yandexdiskuploaderTools
     */
    public function getTools()
    {
        if (!is_object($this->tools)) {
            if ($class = $this->modx->loadClass('tools.yandexdiskuploaderTools', $this->config['handlersPath'], true, true)) {
