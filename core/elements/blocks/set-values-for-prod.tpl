...
{* На некоторых серверах (например, у меня на локалке Open Server) на странице товара дробная цена выводится с одним разделителем (запятая), а в чанке от msProducts - с другим (точка), из-за чего ключ формируется неправильно. Фиксим это, для работы Minishop2 нужна запятая (я проверял) *}
{set $productKey = ($src['id'] ~ ($src['price'] | replace : '.' : ',') ~ $src['weight'] ~ '[]') | replace : ' ' : '' | md5}
