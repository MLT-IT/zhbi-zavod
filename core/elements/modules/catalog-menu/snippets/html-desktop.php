...
foreach ($data as $category_index => $category) {
    ...
    foreach ($category['children'] as $subcategory_name => $childrens) {
        ...
        foreach ($childrens as $children) {
            $html_childrens .= "<a class='catalog-menu__subcategory-item' href='{$children['uri']}'>{$children['title']}</a>";
        }

        $html_subcategory .= "<div class='catalog-menu__subcategory'>
                                    <div class='catalog-menu__subcategory-name fs-14 fw-700 color-red'>$subcategory_name</div>
...
$html_categories = "<div class='catalog-menu__categories'><div class='catalog-menu__categories-name fs-14 fw-700 color-red'>Товар</div>$html_categories</div>";
