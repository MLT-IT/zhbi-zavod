...
if (!class_exists('MLTSearch')) {
    class MLTSearch
    {
        ...
        const TEST_QUERY = null; // Оставить пустым или null - это для тестов
        const TEMPLATE_CATEGORY = 2;
        const TEMPLATE_PRODUCT = 3;
        const TVImageID = 2;
            ...
            if ($this->requestType == self::REQUEST_TYPE_NOT_AJAX) {
                ...
                foreach ($products as $itemIndex => $itemValues) {
                    ...
                    if (count($productsValues) < self::LIMIT_PRODUCTS) {
                        ...
                        $productsValues[$itemIndex]['tv_image'] = $itemValues['tv_image'];
                    }
                }
        ...
        public function generateWhere($tableField)
        {
            ...
            $where = $wherePagetitle . ' OR (`resources`.`pagetitle` LIKE :queryPhrase)';
        ...
        //         `resources`.`pagetitle`
        //     FROM
        //         `{$this->table_prefix}site_content` AS `resources`
        //     WHERE `resources`.`published` = 1 AND `resources`.`deleted` = 0 AND `resources`.`context_key` = '" . $context_key . "' AND `resources`.`template` IN(" . self::SEO_TEMPLATES . ") AND($wherePagetitle)
        //     GROUP BY
        //     resources.id
        ...
        private function findCategories($whereCategories, $queryWordsArray)
        {
            ...
            $query = "SELECT `resources`.`id`, `resources`.`menutitle`, `resources`.`pagetitle`
                  FROM `{$this->table_prefix}site_content` AS `resources` 
                  WHERE `resources`.`template` = " . self::TEMPLATE_CATEGORY . "
                        AND `resources`.`published` = 1 
                        AND `resources`.`deleted` = 0 
                        AND `resources`.`context_key` = '" . $context_key . "'
                        AND ($whereCategories)  
                  GROUP BY resources.id 
                  ORDER BY id
                  LIMIT " . self::LIMIT_CATEGORIES;
