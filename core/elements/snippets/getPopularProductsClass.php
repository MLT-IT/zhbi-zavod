...
class getPopularProductsClass {
    ...
    static protected function krovlya() {
        ...
        if (!in_array(self::$prod->parent, [16788, 11760, 12069, 12070, 12071, 12073, 12074, 16789, 37621, 44676, 44677, 44678, 44679, 44680, 44681, 76923])) {
            self::$debug[] = "Товар не находится в Металлочерепице. Возвращено " . self::$textStd . ".";
            return false;
        }
        ...
        $where = [];
        $optionsValues = ['proizvoditel' => self::$prod->get('proizvoditel'), 'item_thickness' => self::$prod->get('item_thickness'), 'cvet' => self::$prod->get('cvet'), 'pokrytie' => self::$prod->get('pokrytie')];
        ...
        if (!empty($where)) {
            ...
            self::$debug[] = "У товара не заполнен ни производитель, ни толщина, ни цвет, ни покрытие, дополнительное where не составлено.";
        }
