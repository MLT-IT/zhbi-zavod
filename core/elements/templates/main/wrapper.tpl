    ...
    <main class="layout__main" data-template="main">
      <article class="main-screen section">
        <div class="main-screen__container">
          ...
          {include "file:sections/advantages/wrapper.tpl" styleClass='main-screen__advantages'}
        </div>
      </article>
      {if $_modx->resource.context_key not in list ['tagnerud','profnastil','gibkaya-cherepiza','falcevaya-krovlya','metallocherepica']}
        {include "file:sections/listing-brands/wrapper.tpl"}
      {/if}
      {if $_modx->resource.context_key in list ['tagnerud']}
        {include "file:sections/hits/wrapper.tpl"}
      {/if}
      {if $_modx->resource.context_key not in list ['trotuarnaya-plitka','web', 'kraska', 'suhiesmesi', 'fibrofasad', 'plitnye','beltermo','profnastil','gibkaya-cherepiza','falcevaya-krovlya','metallocherepica']}
        {include "file:sections/actions/wrapper.tpl"}
      {/if}
